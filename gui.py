...
def select_file():
    file_path = filedialog.askopenfilename(title="Selected CSV File",filetypes=[("CSV files", "*.csv"), ("Excel files", "*.xlsx;*.xls")])
    if file_path:
        file_label.configure(text=f"Selected CSV File: {os.path.basename(file_path)}")
...
upload_button = CTkButton(file_frame, text="Selected CSV File", command=lambda: select_file())
